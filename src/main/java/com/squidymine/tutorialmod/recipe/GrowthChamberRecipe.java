...
public record GrowthChamberRecipe(Ingredient inputItem, ItemStack output) implements Recipe<GrowthChamberRecipeInput> {
    ...
    // read Recipe JSON files --> new GrowthChamberRecipe instance per file (stores input and output)
    ...
    public RecipeSerializer<?> getSerializer() {
        return ModRecipes.GROWTH_CHAMBER_RECIPE_SERIALIZER;
    }
    ...
    public static class Serializer implements RecipeSerializer<GrowthChamberRecipe> {
        // Looks inside json file for "ingredient" field -> input item, and "result" field -> output item;
            // then these are used to make a new GrowthChamberRecipe
        ...
        // Similar to MapCodec but for client-server sync
