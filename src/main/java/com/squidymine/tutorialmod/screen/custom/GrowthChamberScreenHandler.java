package com.squidymine.tutorialmod.screen.custom;
...
public class GrowthChamberScreenHandler extends ScreenHandler {
    ...
    private final PropertyDelegate propertyDelegate; // Holds progress and max progress
    ...
    public GrowthChamberScreenHandler(int syncId, PlayerInventory playerInventory,
        ...
        addProperties(arrayPropertyDelegate); // Adds progress and maxProgress??
    }

    public boolean isCrafting() {
        return propertyDelegate.get(0) > 0; // If progress > 0, then the player is crafting
    }

    public int getScaledArrowProgress() {
        int progress = this.propertyDelegate.get(0);
        int maxProgress = this.propertyDelegate.get(1); // Max Progress
        int arrowPixelSize = 24; // This is the width in pixels of your arrow (progress bar)

        return maxProgress != 0 && progress != 0 ? progress * arrowPixelSize / maxProgress : 0; // returns pixels of arrow according to the progress
    }
