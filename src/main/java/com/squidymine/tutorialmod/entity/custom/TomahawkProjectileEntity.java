...
public class TomahawkProjectileEntity extends PersistentProjectileEntity {
    ...
    protected ItemStack getDefaultItemStack() {
        return new ItemStack(ModItems.TOMAHAWK); // Item you get when you pick it up from the ground
    }
    ...
    protected void onEntityHit(EntityHitResult entityHitResult) {
        ...
        if (!this.getWorld().isClient()) {
            entity.damage(((ServerWorld) this.getWorld()), this.getDamageSources().thrown(this, this.getOwner()), 4); // does 4 damage to the hit entity
            this.getWorld().sendEntityStatus(this, (byte)3);
            this.discard(); // discards the projectile
        }
    }

    @Override
    protected void onBlockHit(BlockHitResult result) { // ensures that when the tomahawk hits the ground, it is embedded nicely (blade first??)
