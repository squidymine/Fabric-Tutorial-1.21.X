...
public class GrowthChamberBlock extends BlockWithEntity implements BlockEntityProvider {
    ...
    @Override // Scatters items inside blockentity when block is broken
    ...
    @Override // Opens blockentity's screen
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        ...
        return validateTicker(type, ModBlockEntities.GROWTH_CHAMBER_BE,
                (world1, pos, state1, blockEntity) -> blockEntity.tick(world1, pos, state1)); //.tick: called once per tick
    }
}
